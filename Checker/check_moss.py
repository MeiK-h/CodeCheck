...
if __name__ == '__main__':
    ...
    try:
        ...
        with open(os.path.join(result, 'index.html'), 'w') as fr:
            fr.write('''<meta charset='utf-8'>由于 moss 查重系统访问缓慢（可能被墙），此次查重失败。您可以新建一个查重并在另一个时间段重新尝试，或者使用 jplag 引擎''')
    else:
        os.mkdir(result)
        with open(os.path.join(result, 'index.html'), 'w') as fr:
            fr.write('''<meta charset='utf-8'>由于 moss 查重系统可能被墙，我们不提供静态的页面<br>
